...
from gapago.core import config  # noqa: F401  (.env load, LangSmith)

from gapago.graphs.graph import build_graph
from langchain_core.messages import HumanMessage, AIMessage
from gapago.core.llm import AVAILABLE_PROVIDERS, get_llm
from gapago.agents.gap_chat_agent import gap_chat_respond
from gapago.agents.retrieval_agent import preload_models
from gapago.utils.progress import init_progress, drain_progress, cleanup_progress, mark_stage_start, mark_stage_done
from gapago.utils.session_store import init_db as init_session_db, save_session, update_session_status
from gapago.utils import cancel as cancel_registry
...
from gapago.paths import FRONTEND_DIR, LANDING_DIR
...
async def delete_history(filename: str):
    ...
    try:
        ...
        if sid:
            from gapago.utils.session_store import delete_session
            delete_session(sid)
            _chat_histories.pop(sid, None)
    except Exception:
        pass
...
async def analyze(query: str, provider: str = "azure", domain: str = "auto", year_range: str = "auto", output_language: str = "auto", user_id: str = "", fast_mode: bool = False, routing_profile: str = "optimized"):
    ...
    from gapago.core.model_router import ModelRouter
...
async def stream(session_id: str, from_idx: int = 0):
    ...
    if not session:
        ...
        from gapago.utils.session_store import get_session
